...
public class Trie{
	...
	private String search(long binaryAddress, Node currentNode, int inputPrefix){
		int sLength = 0;
		long oneOrZero = -1;
		for(int z = 0; z<32; z++){
			//System.out.print((binaryAddress >>> (31-z)) & 1);
		}
		//System.out.println("entering search");
		for(int i = 0; i < 32; i++){
			sLength = getStrideLength(binaryAddress, 32, i,inputPrefix);
			...
			if(currentNode.getChild(sLength, oneOrZero) == null){
				...
				while(sLength > 1){
					sLength--;
					//System.out.println("trying new sLength: " + sLength + "with "+oneOrZero);
					if(currentNode.getChild(sLength, oneOrZero)!=null){
				...
				currentNode = currentNode.getChild(sLength, oneOrZero);
				//System.out.println("Router: "+currentNode.getNextRouter());
			}
	...
	public void add(long bits, int prefixLength, String nextRouter, int ASPath, int inputPrefix){
		add(bits, root, prefixLength, nextRouter, ASPath, inputPrefix);
	}

	private void add(long prefix, Node currentNode, int length, String nextRouter, int ASPath,int inputPrefix){
		int sLength = 0;
		long oneOrZero = 0;
		//System.out.println("entering");
		for(int i = 0; i < length; i++){	
			sLength = getStrideLength(prefix, length, i,inputPrefix);
			oneOrZero = ((prefix >>> (31-i)) & 1);
			if(currentNode.getChild(sLength, oneOrZero) == null){
				//System.out.println("Adding: " + sLength + " " + oneOrZero);
				currentNode.setChild(new Node(), sLength, oneOrZero);
			}
			//System.out.println("Not Null Node, sLength is: " + sLength + "|" + oneOrZero);
			currentNode = currentNode.getChild(sLength, oneOrZero);
			i += (sLength-1);
		}
	...
	private int getStrideLength(long prefix, int length, int position, int inputPrefix){
		if(((length - position)==1)|| inputPrefix ==1){
			return 1;
		}
		...
		if(sameVal == (prefix >>> (31-(position+1))&1)){
			if((sameVal == ((prefix >>> (31-(position+2))) &1)) && (inputPrefix == 3)){
				return 3;
			}
			return 2;
		}else{	
	...
	private class Node{
		...
		public void setChild(Node child, int sLength, long oneOrZero){
			...
			//System.out.println("added: " + sLength + "|" + oneOrZero);
		}
		public Node getChild(int sLength, long oneOrZero){
			//System.out.println("Looking for: " + sLength + "|" + oneOrZero);
			if(sLength == 1){
				if(oneOrZero == 0){
					return children[0];
				}
