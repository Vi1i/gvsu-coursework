...
def sendMessage(ipAddress):
	...
	if (sendMessage == "EXIT"):
		sys.exit()
	buildMessage = "from="+ownIP+"To=sendIP"+"Message:"+sendMessage
	#Shouldn't the dest be the physical IP address of the router? (ipAddress variable not sendIP)
	sendPacket = IP(version = 4, dst=sendIP, ihl=21) / UDP(dport=5214, len=len(sendMessage)) / buildMessage
	...
	newPacket = IP(version = 4, dst=ipAddress, ihl=21, len = size) / UDP(dport=5214, len=size-20) / buildMessage
