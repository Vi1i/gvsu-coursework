...
class sendThread(threading.Thread):
	def __init__(self, ipAddress, ownIP):
	...
	def run(self):
		try:
			while 1:
				sendMessage(self.ipAddress, ownIP)
		except:
...
def sendMessage(ipAddress, ownIP):
	...
	newPacket = IP(version = 4,src = ownIP, dst=sendIP, ihl=21,ttl=5, len = size) / UDP(dport=5214, len=size-20) / sendMessage 
...
#print sys.version
try:
	receiveThreading = receiveThread()
	sendThreading = sendThread(defSendIP, ownIP)
