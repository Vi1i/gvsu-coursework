/*****************************************************
 * This program is the reader that reads              *
 * shared memory segments from the                    *
 * wrier.c program. It displays whatever is in the    *
 * shared memory.                                     *
 *                                                    *
...
int main (){ 
	...
	while(1) {
		...
		ptr += *p++;
		input[2] = ptr;
		printf("I got:  %s\n", input[0]); //prints to the screen what's in shared mem
		sprintf(ptr, "0");
		sscanf(ptr, "%d", &p1);
